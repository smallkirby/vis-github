...
fn fetch_repositories_from_file(owner: &str, cache_dir: &str) -> Result<Vec<Repository>, String> {
  ...
  if !basedir.exists() || !basedir.is_dir() {
    return Err(format!("Cache path for repo({}) not found.", basedir.to_string_lossy()));
  }
  ...
  for repo in repos {
    ...
    if !repo_path.exists() || !repo_path.is_dir() {
      if fs::create_dir(&repo_path).is_err() {
        return Err(format!("Failed to create cache directory: {}", repo_path.to_string_lossy()).into());
      }
    }
    repo_path.push("repo.json");
    match fs::write(&repo_path, serde_json::to_string(repo).unwrap()) {
      Ok(()) => continue,
      Err(err) => return Err(format!("Failed to create repo cache: {} : {}", repo_path.to_string_lossy(), err.to_string())),
    }
  }
